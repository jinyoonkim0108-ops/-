...
import { Search, X, Triangle, Globe, ShoppingCart, User, Heart, Menu } from 'lucide-react';
...
const Header: React.FC<HeaderProps> = ({ lang, onLanguageChange, t }) => {
  ...
  const renderRankList = (items: any[], title: string) => (
    <div className="flex-1">
      <div className="flex items-center justify-between mb-6">
        <h3 className="text-xl font-black text-gray-900">
          실시간 <span className="text-pink-600">{title}</span>
        </h3>
      </div>
        ...
        <div className="space-y-4">
          {items.slice(0, 10).map((item, i) => (
            <div key={i} className="flex items-center group cursor-pointer">
              ...
              <span className="flex-grow text-sm font-bold text-gray-700 group-hover:text-pink-600 transition-colors ml-2 truncate">
                {item.term || item.name}
              </span>
              <span className="ml-2">
                {item.change === 0 ? (
                  ...
                  <Triangle className="w-2 h-2 fill-red-500 text-red-500" />
                ) : (
                  <Triangle className="w-2 h-2 fill-blue-500 text-blue-500 rotate-180" />
                )}
              </span>
            </div>
          ))}
      ...
      <header className="bg-white sticky top-0 z-50 shadow-sm">
        ...
        <div className="max-w-[1400px] mx-auto px-4 py-4 flex items-center justify-between">
          ...
          <div className="flex-grow max-w-xl mx-8 relative">
            <input
              type="text"
              placeholder="봄 신상 단독 오픈 특가 상품을 만나보세요"
          ...
          <div className="flex items-center space-x-6 text-sm font-bold text-gray-900">
            <button className="hover:text-pink-600">골프웨어</button>
            <button className="hover:text-pink-600">아웃렛</button>
        ...
        <nav className="border-t border-gray-100">
          <div className="max-w-[1400px] mx-auto px-4 flex items-center h-14 space-x-8 text-[15px] font-bold text-gray-800">
            <button className="flex items-center gap-2 text-black"><Menu className="w-5 h-5" /> 전체카테고리</button>
            <button className="hover:text-pink-600">여성</button>
            <button className="hover:text-pink-600">남성</button>
            <button className="hover:text-pink-600">유아동</button>
            <button className="hover:text-pink-600">스포츠</button>
            <button className="hover:text-pink-600">라이프</button>
            <div className="w-[1px] h-4 bg-gray-200"></div>
            <button className="text-gray-500 hover:text-black">베스트</button>
            <button className="text-gray-500 hover:text-black">브랜드</button>
          </div>
        </nav>
      </header>
